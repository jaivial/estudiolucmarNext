...
export default async function handler(req, res) {
    ...
    try {
        console.time("Existing Agrupacion Duration");
        ...
        for (const nestedInmueble of nestedInmuebles) {
            ...
            const updateResult = await db.collection('inmuebles').updateOne(
                { id: existingGroup },
                { $push: { nestedinmuebles: nestedInmueble } }
            );
        ...
        const updatedDocument = await db.collection('inmuebles').findOne({ id: existingGroup });
        console.log('Updated document:', updatedDocument);

        console.timeEnd("Existing Agrupacion Duration");
